...
mod build {
    ...
    const BUILD_HASH_FILE_ENV: &str = "BUILD_HASH_FILE";
    ...
    pub(crate) fn build() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let info = compile_from_source(dir);
    ...
    pub fn compile_from_source(dir: PathBuf) -> BuildInfo {
        let url = format!(
        ...
        if cfg!(target_os = "windows") {
            ...
            cmd.arg("build.bat");
            cmd.arg("--build_dir");
            cmd.arg(windows_fix_path(&build_dir));
        } else {
    ...
    /// Downloads a file from an url and dumps it into `output_path`. It first
    /// downloads the content into a temporal file and then moves it to the
    /// final destination, reporting the progress in the way.
    ...
    /// Extracts a .tar.gz file into the provided `output_dir`.
    fn extract_tar_gz(archive_path: &str, output_dir: &str) {
        ...
        let entries_count =
            must!(archive.entries(), "Error retrieving entries from tar file").count();
    ...
    /// Runs a command in the system redirecting stdout and stderr
    /// to the current Rust process.
    ...
    /// Calculates a hash out of anything that can be stringified.
    fn calculate_hash(input: impl Display) -> String {
        // Create a Sha256 object
        let mut hasher = DefaultHasher::new();

        // Write input data
        hasher.write(input.to_string().as_ref());
    ...
    /// When serializing paths on windows in order to provide them as
    /// Command arguments, passing them through this function seems
    /// necessary, as otherwise it fails saying that the path cannot be found.
